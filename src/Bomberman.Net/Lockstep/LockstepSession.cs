...
namespace Bomberman.Net.Lockstep
{
    ...
    public sealed class LockstepSession
    {
        ...
        ///
        /// Anchoring the horizon to <see cref="CurrentFrame"/> (rather than blindly bumping a counter
        /// every call) is what makes the input delay both CONSISTENT and SELF-HEALING:
        ...
        ///    delay as large as the stall was long; the lead can no longer shrink back. Capping it means
        ///    the input delay snaps back to normal the instant lockstep resumes.
        /// Each frame is written exactly once, so a committed input is never mutated (mutating an input the
        /// peer already simulated would desync it). Re-sending unchanged frames is safe and helps the peer
        /// recover packets dropped during a freeze.</summary>
